...
### Hybrid JS + TS setup
Runtime entry, router, utils, data, composables, and theme are plain JS (`main.js`, `*.js`), but every SFC uses `<script setup lang="ts">` and each JS module ships a sibling `.d.ts` declaration file. Follow this pattern: JS implementation + `.d.ts` types. Vite aliases: `@` → `src`, `@components` → `src/components`, `@utils` → `src/utils`. All three are also configured in `tsconfig.json` paths.
...
- `ScaleRing.vue` — static annotation ring: trigram symbols (r=236), 宫名+五行 (r=254), heavenly stems / earthly branches (r=272).
...
- `ResultScroll.vue` — the sliding scroll panel that appears in reading state, containing a `ScrollContent` instance for the divination result.
- `ScrollContent.vue` — renders the divination result (卦辞/爻辞/白话解读) in a traditional scroll aesthetic. Reads `direction` from the useCompass composable.
