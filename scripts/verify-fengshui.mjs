...
// 年界精确：2025 立春 2/3，当日即换乙巳年
check(yearGanZhiAt(2025, 2, 3).name === '乙巳', '2025-02-03 立春当日应乙巳年');
check(yearGanZhiAt(2025, 2, 2).name === '甲辰', '2025-02-02 立春前应甲辰年');

// —— 精确节气（太阳视黄经反推）——
// 太阳黄经：2024 立春交节 16:26 CST，16:00 时应略小于 315°
check(
  Math.abs(sunLongitude(localJd(2024, 2, 4, 16)) - 315) < 0.1,
  '2024 立春 16:00 太阳黄经应 ≈315°'
);
const jieDate = (year, lon) => {
  const mo = jieMoment(year, lon);
  return `${mo.y}-${mo.m}-${mo.d}`;
};
check(jieDate(2024, 315) === '2024-2-4', '2024 立春应 2/4');
check(
  jieDate(2024, 15) === '2024-4-4',
  '2024 清明应 4/4（精确，非近似表 4/5）'
);
check(jieDate(2024, 0) === '2024-3-20', '2024 春分应 3/20');
check(jieDate(2024, 270) === '2024-12-21', '2024 冬至应 12/21');
check(jieDate(2024, 345) === '2024-3-5', '2024 惊蛰应 3/5');
check(jieDate(2024, 285) === '2024-1-6', '2024 小寒应 1/6');
check(jieDate(2024, 255) === '2024-12-6', '2024 大雪应 12/6');
check(jieDate(2025, 315) === '2025-2-3', '2025 立春应 2/3');
check(jieDate(1984, 315) === '1984-2-4', '1984 立春应 2/4');
...
// 交节当日换月（近似表边界 ±1 天，精确后清明 4/4 即换辰月）
check(jieZhi(2024, 4, 4) === '辰', '2024-04-04 清明当日应辰月');
check(jieZhi(2024, 4, 3) === '卯', '2024-04-03 清明前应卯月');
check(jieZhi(2025, 2, 3) === '寅', '2025-02-03 立春当日应寅月');
check(jieZhi(2025, 2, 2) === '丑', '2025-02-02 立春前应丑月');
